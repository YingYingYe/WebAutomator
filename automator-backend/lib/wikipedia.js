...
async function getWikiCoord(name) {
  ...
  try {
    ...
    try{
      ...
      const coord_arr = coord.split("|");
      if (coord_arr.length >= 8){
        long = coord_arr[0] + "° " + coord_arr[1] + "′ " + coord_arr[2] + "″ " + coord_arr[3];
        lat = coord_arr[4] + "° " + coord_arr[5] + "′ " + coord_arr[6] + "″ " + coord_arr[7];
      }
    }catch(ex){
      long = "";
      lat = "";
